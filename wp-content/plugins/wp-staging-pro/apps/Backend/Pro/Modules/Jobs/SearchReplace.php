...
class SearchReplace extends \WPStaging\Backend\Modules\Jobs\JobExecutable {
    ...
    private function getDestinationHost() {
        $helper          = new Helper();
        $destinationHost = $helper->get_home_url_without_scheme();

        if( $this->isSubDir() ) {
            $destinationHost = $helper->get_home_url_without_scheme() . '/' . $this->getSubDir();
        }
...
//        $args['search_for'] = array(
//            str_replace( '/', '\/', $this->get_url_without_scheme( $this->options->url ) ), //host.com
...
//        );
        ...
        //$this->debugLog( 'Search: ' . $this->get_url_without_scheme( $this->options->url ) . ' Replace: ' . $this->get_url_without_scheme( $helper->get_home_url() ) );
