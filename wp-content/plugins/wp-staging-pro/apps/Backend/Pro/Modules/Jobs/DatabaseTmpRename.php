...
class DatabaseTmpRename extends \WPStaging\Backend\Modules\Jobs\JobExecutable {
    ...
    protected function checkFatalError() {
        if( $this->tmpPrefix == $this->db->prefix ) {
            $this->returnException( 'Fatal Error: Can not replace live tables with staging ones because table prefix of both sites is the same. Please try again or contact support@wp-staging.com' );
        }
    }
    ...
    protected function renameTable( $tmp ) {
        ...
        if( $this->isTable( $liveTable ) ) {
            if( false === $this->db->query( "RENAME TABLE {$liveTable} TO {$backupTable}, {$tmpTable} TO {$liveTable}" ) ) {
                ...
                $this->log( "DB Rename: Error - Can not rename table {$liveTable} TO {$backupTable} and {$tmpTable} TO {$liveTable} Error: {$this->db->last_error}", \WPStaging\Utils\Logger::TYPE_ERROR );
                return false;
            ...
            if( false === $this->db->query( "RENAME TABLE {$tmpTable} TO {$liveTable}" ) ) {
                //$this->db->query("ROLLBACK");
                $this->log( "DB Rename: Error - Can not rename table {$tmpTable} TO {$liveTable} Error: {$this->db->last_error}", \WPStaging\Utils\Logger::TYPE_ERROR );
                return false;
