...
//        }
    ...
    private function getExternalDBConnection() {
        ...
        if( empty( $results ) ) {
            $this->returnException( "Database {$this->options->databaseDatabase} does not exist. You need to create it first." );
            return false;
        }
        return $db;
    }
    ...
    private function startJob( $new, $old ) {
        ...
        //$this->stagingDb->query( "CREATE TABLE `{$this->options->databaseDatabase}`.`{$new}` LIKE `{$this->db->dbname}`.`{$old}`" );
        $this->stagingDb->query( $sql );
    ...
    private function getCreateStatement( $tableName ) {
        //$result = $this->mysql->query( "SHOW CREATE TABLE `{$tableName}`" );
