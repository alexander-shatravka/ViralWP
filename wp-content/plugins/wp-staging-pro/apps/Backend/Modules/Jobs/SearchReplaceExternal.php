...
//      }
    ...
    private function getSourceHostname() {
        ...
        if( $this->isSubDir() ) {
            $host = $this->options->homeHostname . '/' . $this->getSubDir();
        }
        ...
        foreach ( $data as $row ) {
            ...
            foreach ( $columns as $column ) {
                ...
                if( false === strpos( $dataRow, $tmp['search_for'][0] ) ) {
                    array_shift( $tmp['search_for'] ); // rtrim( $this->options->sourceHostname, '/' ),
                    array_shift( $tmp['replace_with'] ); // rtrim( $this->options->sourceHostname, '/' ) . '/' . $this->options->cloneDirectoryName,
                } else {
